...
def Attend():
  ...
  while True:
    ...
    conn = mariadb.connect(user='ft', password='future12',
...
def Adduser():
  if request.method=='POST':
    ...
    conn = mariadb.connect(user='ft', password='future12',db='Attended', host='localhost')
    c = conn.cursor()
...
def Report():
  if request.method == "POST":
    ...
    c = conn.cursor(buffered=True)
    cur = conn.cursor()
    cur.execute("SELECT * FROM attended  WHERE E_date between '{}' and '{}' and A_name='{}'".format(Data_start, name,nam))
